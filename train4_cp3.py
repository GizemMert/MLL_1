...
umap_dir = 'umap_figures4cp3'
if not os.path.exists(umap_dir):
    os.makedirs(umap_dir)

latent_dir = 'latent_data4cp3'
if not os.path.exists(latent_dir):
    os.makedirs(latent_dir)

result_dir = "training_results4cp3"
os.makedirs(result_dir, exist_ok=True)
result_file = os.path.join(result_dir, "training_results4cp3.txt")
...
def calculate_psnr(kornia_psnr, im_out, scimg,  max_pixel=1.0):
    # Assuming original and reconstructed are torch Tensors with pixel values in [0, 1]
    psnr_value = kornia_psnr(im_out, scimg, max_val=max_pixel)
    return psnr_value
...
for epoch in range(epochs):
    ...
    for i in range(30):
        ...
        if epoch % 10 == 0:
            file_name = "reconsructed-images4_cp3/"
...
model_save_path = os.path.join(script_dir, 'trained_model4cp3.pth')
