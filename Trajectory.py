...
def interpolate_gif_gpr(model, filename, latent_start, latent_end, steps=100, grid_size=(1, 100), device=device):
    model_1.eval()
...
# plt.legend()
plt.xlim(0, sz - 1)
plt.savefig(os.path.join(umap_dir, 'gene_expression_fold_change_trajectory_by_cluster.svg', dpi=400))
plt.tight_layout()
plt.close()
