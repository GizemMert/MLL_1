...
monocyte_z_dir = 'z_data4cp2_new5_std_gen'
if not os.path.exists(monocyte_z_dir):
    os.makedirs(monocyte_z_dir)

myle_z_dir = 'z_data4cp2_new5_std_gen'
...
for epoch in range(epochs):
    ...
    for feat, scimg, mask, label, _ in train_dataloader:
        ...
        if neutrophil_mask.any():
            ...
            mmd_loss_neutrophil = MMDLoss()(z_neutrophil, ref_z_class_2)
        ...
        if myeloblast_mask.any():
            ...
            mmd_loss_myle = MMDLoss()(z_myle, ref_z_class_2)
    ...
    file_name = "reconstructed-myeloblast/"
    if not os.path.exists(file_name):
        os.makedirs(file_name)
